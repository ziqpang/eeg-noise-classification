...
def plot_attention_heatmap(attention_weights, save_dir, fold):
    """Plot attention weights heatmap"""
    plt.figure(figsize=(10, 6))
    sns.heatmap(attention_weights.squeeze(), cmap='viridis')
    plt.title(f'Attention Weights Distribution (Fold {fold})')
    plt.xlabel('Time Steps')
    plt.ylabel('Samples')
    plt.savefig(os.path.join(save_dir, f'attention_heatmap_fold_{fold}.png'))
    plt.close()
...
def plot_fold_boxplots(fold_results, class_accuracies, save_dir):
    ...
    plt.title('CNN_LSTM_Model Accuracy Distribution by Class')
    plt.ylabel('Accuracy')
    plt.grid(True, alpha=0.3)
    plt.ylim(0.6, 1)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'CNN_LSTM_Model_accuracy_boxplot.png'))
    plt.close()
...
def plot_roc_curves(fold_results, save_dir):
    ...
    plt.title('CNN_LSTM_Model ROC Curves (All Folds)')
    plt.legend(loc='lower right')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'CNN_LSTM_Model_roc_curves.png'))
    plt.close()
...
def plot_aggregated_results(fold_results, save_dir, class_accuracies):
    ...
    with open(os.path.join(save_dir, 'CNN_LSTM_Model_summary.txt'), 'w') as f:
        f.write("CNN_LSTM_Model Performance Summary\n")
        f.write("==================================================\n\n")
    ...
    plt.title('CNN_LSTM_Model Average Confusion Matrix (%)')
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.savefig(os.path.join(save_dir, 'CNN_LSTM_Model_avg_confusion_matrix.png'))
    plt.close()
    ...
    # t-SNE visualization of aggregated predicted probabilities
    ...
    plt.title('CNN_LSTM_Model t-SNE of Predicted Probabilities')
    plt.legend(title='Class')
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'CNN_LSTM_Model_tsne.png'))
    plt.close()

def main():
    # Create results directory
    results_dir = '/root/autodl-tmp/DeepSeparator-main/results/cnn_lstm_model'
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(signals, labels)):
        ...
        for epoch in range(num_epochs):
            ...
            # consider significant improvements only
    ...
    avg_acc = np.mean([result['best_val_acc'] for result in fold_results])
    std_acc = np.std([result['best_val_acc'] for result in fold_results])
    print("\nCNN_LSTM_Model Performance Summary")
    print("==================================================")
    print("\nOverall Performance:")
